  ...
  async function chooseTrack(slug: string) {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setTrackChosen(slug);
        if (data.goalsGenerated) setGoalsGenerated(true);
        router.refresh();
      }
    } finally {
  ...
  if (matches) {
    return (
      <div style={{ animation: "var(--animate-fade-up)" }}>
        ...
        {goalsGenerated && (
          <div className="mt-6 flex items-start gap-2 rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
            <Check className="mt-0.5 h-4 w-4 shrink-0" />
            <span>{t("goalsAutoGenerated")}</span>
          </div>
        )}
        <button onClick={restart} className="btn-ghost mt-4">
